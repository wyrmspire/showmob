...
globs_for_area() {
    case "$1" in
        ...
        supabase)
            # Persistence foundation slice for agent handoff (no secrets).
            printf '%s\n' \
                "supabase/**" \
                "app/src/persistence/**" \
                "scripts/milestone-one-check.mjs" \
        ...
        *)
            ...
            echo "Valid areas: backend frontend docs scripts plugins tests config supabase" >&2
            exit 1
            ;;
    esac
}
...
AREA PRESETS (--area, repeatable)
  ...
  supabase  supabase/** + persistence adapter, milestone check,
            supabase tests, .env.example, ROADMAP.md
            (prefer ./printcode-supa.sh)
...
EXAMPLES
  ...
  # 11) Supabase foundation slice (or use ./printcode-supa.sh)
  ./printcode.sh --area supabase --output-prefix dump-supa
EOF
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --area)
            ...
            case "$2" in
                backend|frontend|docs|scripts|plugins|tests|config|supabase) ;;
...
DEFAULT_EXTS=(py sh md yaml yml ts tsx css toml json ini js mjs html svg)
...
priority_for_path() {
    ...
    case "$rel_path" in
        ...
        app/src/components/BlockView.tsx)
            echo "00"
            ;;
        app/src/*|\
        docs/*|\
...
for ai in "${ALWAYS_INCLUDE[@]}"; do
    if [[ -f "$PROJECT_ROOT/$ai" ]] && ! grep -Fxq "$ai" "$FILE_LIST"; then
        echo "$ai" >> "$FILE_LIST"
    fi
done
...
# ---------------------------------------------------------------------------
{
    ...
    NEXUS_DIR="${NEXUS_DIR:-/c/notes}"
    if [[ -d "$NEXUS_DIR" ]]; then
        echo "- **Nexus:** found at $NEXUS_DIR (not appended in Showmob dumps by default)"
    else
        echo "- **Nexus:** not found (looked for $NEXUS_DIR) — Showmob-only dump"
    fi
    echo ""
} > "$TEMP_FILE"
...
# ---------------------------------------------------------------------------
{
    ...
    echo ""
    echo "Showmob turns structured ideas into portable web pages. Browse is the default reading view;"
    echo "presentation lives as embedded slideshow blocks. Vite + React; JSON artifacts under app/src/content/."
    echo ""
    echo "| Area | Path | Description |"
    echo "|------|------|-------------|"
    echo "| **app** | app/src/ | Renderer, schema, validation, studio |"
    echo "| **content** | app/src/content/ | schemaVersion 1 JSON artifacts |"
    echo "| **docs** | docs/ | Architecture, blueprint, roadmap, widgets |"
    echo "| **tests** | tests/ | Content/validation suite |"
    echo "| **public** | public/ | Static assets |"
    echo ""
    echo "Key paths: \`app/src/App.tsx\`, \`app/src/schema.ts\`, \`app/src/validation.ts\`, \`AGENTS.md\`"
    echo "Stack: Vite, React, TypeScript — live: https://showmob.vercel.app"
    echo ""
    echo "To dump specific code for chat context, run:"
    echo "\`\`\`bash"
    echo "./printcode.sh --help"
    echo "./printcode.sh --path \"app/src/**\" --ext ts,tsx"
    echo "./printcode.sh --list --area docs"
    echo "\`\`\`"
    echo ""
} >> "$TEMP_FILE"
# ---------------------------------------------------------------------------
...
if $SHOW_STRUCTURE; then
    ...
    if [[ ${#INCLUDE_PATTERNS[@]} -gt 0 ]] || [[ ${#EXT_FILTER[@]} -gt 0 ]] || [[ ${#USER_EXCLUDES[@]} -gt 0 ]]; then
        ...
        # Default dump: structure matches selected file bodies
        cat "$FILE_LIST" >> "$TEMP_FILE"
    fi
    echo '```' >> "$TEMP_FILE"
    echo "" >> "$TEMP_FILE"
...
# ---------------------------------------------------------------------------
# Optional Nexus note (not part of Showmob)
# ---------------------------------------------------------------------------
NEXUS_DIR="${NEXUS_DIR:-/c/notes}"
{
    echo "## Nexus status"
    echo ""
    if [[ -d "$NEXUS_DIR" ]]; then
        echo "**Nexus: found** at \`$NEXUS_DIR\`."
        echo "This Showmob dump does not append Nexus file bodies by default."
    else
        echo "**Nexus: not found** (looked for \`$NEXUS_DIR\`)."
        echo "This is a **Showmob-only** dump."
    fi
    echo ""
} >> "$TEMP_FILE"
